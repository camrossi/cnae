...
class TopologyExplorer(object):
    ...
    def __init__(self, logger, apic_ips,
        ...
        self.connection_preference_primary = ConnectionPreference.OOB
    ...
    def getResolvedHost(self, host_name):
        try:
            resolved_host = gethostbyname(host_name)
        except BaseException:
        ...
        for dcnm in self.sa_topo_args.dcnms:
            try:
                ...
                for node_info in json.loads(fabric_nodes_rsp):
                    if node_info["fabricName"].lower() != self.sa_topo_args.sa_fabric.lower():
                        continue
                    ip = node_info['ipAddress']
                    node_name = node_info['logicalName']
                    node_role = node_info['switchRole']
                    node_ip = self.getResolvedHost(ip)
                    ...
                    if update_version is True:
                        ...
                        self.topology.updateTopoNode(node_ip, "version",
                                                     version[0])
    ...
    def addFabricNode(self, fabric_node, configured_ip_map, top_system_oob_ip_map, top_system_inband_ip_map):
        ...
        elif self.getConnectionPreferencePrimary() == ConnectionPreference.OOB:
            node_ip  = node_oob_ip
...
class GenericCollector:
    ...
    class StandaloneQueries:
        ...
        @staticmethod
        def getQueries(node_role=None, sa_fabric=None):
            queries = []
            if node_role in [STANDALONE_LEAF]:
                queries = [
                    ...
                    RestQuery(QueryId.NX_FABRIC_BD, '/api/mo/sys/bd.json?rsp-subtree=full',
                              standalone = True),
                    SshQuery(QueryId.NX_FABRIC_SHOW_LLDP_NEIGHBORS, "show lldp neighbors system-detail | json",
                ...
                queries = [
                    RestQuery(QueryId.NX_FABRIC_DCNM_INVENTORY, '/rest/control/fabrics/{0}/inventory'.format(sa_fabric),
                              standalone = True),
    ...
    def check_if_node_is_reachable(self, node):
        try:
            proc = subprocess.Popen(['ping', '-c', '1', node.getNodeIp()],
    ...
    def initializeConnection(
        ...
        if node.isVCenter():  # vCenter
            ...
            url = '%s://%s:%s' % (protocol, node.getNodeIp(), port)
            self.logger.info(
                "NodeId: %d Starting DCNM REST session name:%s ip:%s role:%s" %
                (node.node_id, node.node_name, node.getNodeIp(), node.node_role))
            try:
                session = DCNMLoginSession(url, user, password, timeout=node_time_out,
                                           request_format='json')
                rest_access = DirectRestAccess(session)
                rest_access.login()
            except Exception as e:
    ...
    def queryRest(self, rest_access, node, query):
        ...
        self.logger.info("features needed enabled for query %s hence executing query" %(query.getQueryCmd()))
        max_retries = 2
        base_url = '%s://%s:%s' % (self.protocol, node.getNodeIp(), self.port)
    ...
    def hasError(self, bytes_stdout):
        errors = ["Error, procket", "Cmd exec error",
    ...
    def querySsh(self, ssh, node, query, query_time_out):
        ...
        while max_retries > 0:
            try:
                ...
                if not ssh.isActive():
                    self.logger.info(
                        "compareTopoDisc.pyNodeId: %d restarting unactive SSH session name:%s ip:%s role:%s" %
                        (node.node_id, node.node_name, node.getNodeIp(), node.node_role))
...
class ApicExportConfigurationCollector(GenericCollector):
    ...
    def setTopoNode(self, node):
        self.node = node
        self.setTopoNodeList([node])

    def getAndUpdateCollectionTime(self):
        if self.node_collection_stats:
            with self.stats_lock:
                collection_time_elapsed = (
                    round(time.time())) - self.node_collection_stats.getCollectionStartTime()
                collection_time_remaining = self.export_policy_time_out - collection_time_elapsed
                self.node_collection_stats.current_collection_time = collection_time_elapsed
    ...
    def remoteCopyConfigExportTarGz(self, query):
        ...
        if self.export_job_filename and self.remote_path:
            ...
            self.querySftp(self.ssh, self.node, query)
        ...
        try:
            ...
            if ccg.get("cnaeMode") == _APIC and not controllers:
                ...
                if export_policy_name and export_policy_format and export_policy_timeout:
                    ...
                    config_export.setTopoNode(controllers[0])
...
def runCandidDataCollectionOffline():
    ...
    if args.cnaeMode == _APIC:
        ...
        # args.__dict__["connectionPreferenceSecondary"] = "outofband"
